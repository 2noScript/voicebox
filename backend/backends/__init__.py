...
# On macOS + MLX only Qwen TTS (via mlx-audio) is available.
TTS_ENGINES = {
    "qwen": "Qwen TTS",
}
...
def _get_qwen_model_configs() -> list[ModelConfig]:
    """Return Qwen model configs (MLX repos only)."""
...
def _get_qwen_custom_voice_configs() -> list[ModelConfig]:
    """Return Qwen CustomVoice model configs (removed — macOS + MLX only)."""
    return []


def _get_non_qwen_tts_configs() -> list[ModelConfig]:
    """Return model configs for non-Qwen TTS engines (removed — macOS + MLX only)."""
    return []
...
def _get_qwen_llm_configs() -> list[ModelConfig]:
    """Return Qwen3 LLM configs (MLX 4-bit repos only)."""
